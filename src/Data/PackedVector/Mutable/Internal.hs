...
module Data.PackedVector.Mutable.Internal (
  DynamicNat(..)
, MutablePackedVector(..)
, MPVector(..)
-- Creation
...
import Data.Proxy
import qualified Data.PackedVector.Util as Util
...
instance
  ...
  foldMapI f b1 v@(MPVec_SS fptr) = Util.foldMap elemsPerWord (length v) fptr b1 f
    where
      elemsPerWord = fromIntegral . natVal $ Proxy @(Div 64 bitSize)
  {-# INLINE foldMapI #-}
...
unsafeCopy dst src
  ...
  | otherwise   = unsafePrimToPrim $ itraverse_ (\i a -> unsafeWrite @IO ioDst i a) ioSrc
  where
